export default (function () {
    var _intPart = function (floatNum) {
    ...
    var _gregorianToHijriConversion = function (date) {
        var dateArray = date.split('/');
        var d = parseInt(dateArray[0], 10);
        ...
        var n = _intPart((l - 1) / 10631);
        l = l - 10631 * n + 354;
        var j = (_intPart((10985 - l) / 5316)) * (_intPart((50 * l) / 17719)) + (_intPart(l / 5670)) * (_intPart((43 * l) / 15238));
    ...
    var _hijriToGregorianConversion = function (date) {
        var dateArray = date.split('/');
        var y = parseInt(dateArray[0], 10);
        var m = parseInt(dateArray[1], 10);
        var d = parseInt(dateArray[2], 10);
        var jd = _intPart((11 * y + 3) / 30) + 354 * y + 30 * m - _intPart((m - 1) / 2) + d + 1948440 - 385;
        var l, n, i, j, k;
    ...
    var getIslamicDate = function (date) {
        var hijriDate = _gregorianToHijriConversion(date);
        var month = hijriDate.month.toString().length === 2 ? hijriDate.month : ['0', hijriDate.month].join('');
        var day = hijriDate.day.toString().length === 2 ? hijriDate.day : ['0', hijriDate.day].join('');

        return [hijriDate.year, month, day].join('/');
    };

    var getGregorianDate = function (date) {
        var gregorianDate = _hijriToGregorianConversion(date);
        var formattedGregorianDate = [gregorianDate.year, gregorianDate.month, gregorianDate.day].join('/');
